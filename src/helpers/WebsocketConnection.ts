...
export class WSConnection extends MultiListenable<WSEvent, any> {
                ...
                if (Request.isInstance(message)) {
                    ...
                    if (this.requestHandlers.has(message.requestName)) {
                        ...
                        try {
                            ...
                            response = new ErrorResponse('ServerError', 'An unexpected error occurred while processing this request.');
                        }

                        //Response is either a SuccessResponse, an ErrorResponse or a promise resolving to either

                        const processResponse = (response: SuccessResponse | ErrorResponse) => {
                            if (SuccessResponse.isInstance(response)) {
                                let status = 'okay';
                        ...
                        if (isPromise(response)) {
                            response.then(processResponse).catch((error) => {
                                ...
                                //Return a default error object to the client
                                processResponse(new ErrorResponse('ServerError', 'An unexpected error occurred while processing this request.'));
                            });
                    ...
                    if (ErrorResponse.isInstance(message)) {
                        callback.reject(message);
                    } else if (SuccessResponse.isInstance(message)) {
    ...
    private pendingRequests : Map<number, {resolve: (res: SuccessResponse) => void, reject: (err: ErrorResponse) => void}> = new Map(); //Maps outgoing request IDs to their response promises
     ...
     */
    sendRequest(requestName: string, data?:any) : Promise<SuccessResponse> {
        let resPromiseResolver;
        //Pull the resolve and reject callbacks out of the promise
        let resPromise = new Promise<SuccessResponse>((resolve, reject) => resPromiseResolver = {resolve: resolve, reject: reject});
    ...
    setGuardedRequestHandler<T>(requestName: string, typeGuard: (something: any) => something is T, handler: (data: T) => WSPendingResponse) {
        ...
        let wrappedHandler : WSReqHandler = (data) => {
            if (typeGuard(data)) {
                ...
                return new ErrorResponse('InvalidType', 'Invalid data type for request');
            }
        }
